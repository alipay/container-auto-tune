...
public abstract class AbstractJDK8GCLogParser extends AbstractGCLogParser {
    ...
    private static final String[] TRACETIME_OTHER_TITLES = Arrays.stream(new String[]{
            ...
            "Metaspace Dump (after full gc): ",
            "Heap Dump (before full gc): ",
            "Class Histogram (before full gc): ",
            "Metaspace Dump (before full gc): ",
            "per-gen-adjust",
            "marking",
            "ref processing",
            "adjust-strong-roots",
    ...
    private static final String[][] EMBEDDED_SENTENCE_BEGIN_END = {
            {"[1 CMS-", ")]"},
            {"(Elastic heap concurrent cycle ends)", null},
            {"[YG occupancy: ", "]"},
    ...
    // 2021-05-16T19:59:25.498+0800: 171152.504: [Full GC (System.gc()) (Elastic heap concurrent cycle ends) 1863M->404M(3808M), 1.9253379 secs]
    ...
    private static class LineAssembler {
        ...
        private void doAssemble() {
            if (checkNextToken(TOKEN_LINE_FULL_SENTENCE)) {
                parser.pushSentenceToParse(Lists.newArrayList(lastToken));
            }
            while (!endOfLine()) {
    ...
    protected static final ParseRule cpuTimeRule = new PrefixAndValueParseRule(" [Times",
            ((parser, context, prefix, value) -> {
                List<GCEventType> eventTypes = Lists.newArrayList(((AbstractJDK8GCLogParser) parser).getCPUTimeGCEvent());
