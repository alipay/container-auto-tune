 ...
 */
@Slf4j
public class EvaluateBenefitChecker extends EventChecker {

    private Function<TunePipeline, TuneContext> func;

    public EvaluateBenefitChecker(ApplicationContext applicationContext, TunePipeline tunePipeline) {
    ...
    public TuneStage tuneStage() {
        return TuneStage.EVALUATE_BENEFIT;
    }
    ...
    public void submitNext() {
        try {
            //提交评估
            tuneEffectService.asyncSubmitTunePredict(this.tunePipeline.getAppId(),
                this.tunePipeline.getPipelineId());
        } catch (Exception e) {
            log.error("evaluateBenefitChecker asyncSubmitTunePredict is error", e);
        }
        //决策下一步的状态
