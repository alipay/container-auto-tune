 ...
 */
@Slf4j
public class VerifyEffectChecker extends EventChecker {

    public VerifyEffectChecker(ApplicationContext applicationContext, TunePipeline tunePipeline) {
        super(applicationContext, tunePipeline);
    }

    @Override
    public TuneStage tuneStage() {
        return TuneStage.VERIFY_EFFECT;
    }
    ...
    public void submitNext() {
        ...
        try {
            //提交评估
            tuneEffectService.asyncTuneEffect(this.tunePipeline.getPipelineId(), "effect");
        } catch (Exception e) {
            log.error("verifyEffectChecker asyncTuneEffect is error", e);
        }
        //更新下tune_plan的状态
        tunePlanRepository.updateTuneStatusById(tunePipeline.getTunePlanId(), TunePlanStatus.END);
        //决策下一步的状态
