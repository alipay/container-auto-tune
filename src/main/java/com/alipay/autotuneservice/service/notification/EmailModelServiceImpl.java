...
public class EmailModelServiceImpl implements EmailModelService {
    ...
    public void tuneSubmitEmail(Integer pipelineId, Integer appId) {
        try {
            ...
            String content = String.format(
                    "hello,  %s starts a new tuning process at %s,  <a href=\"%s\">please click for details</a>", getAppName(appId),
                    getCurrentDate(), url);
            emailService.sendMail(Arrays.asList("xx@qq.com"), content, SUBJECT);
        } catch (Exception e) {
    ...
    public void tunePredictEmail(Integer pipelineId, Integer appId) {
        try {
            ...
            //todo 获取用户邮箱 发送邮箱通知
            String content = String.format(
                    "hello,  %s starts expected evaluation at %s,  <a href=\"%s\">please click for details</a>", getAppName(appId),
                    getCurrentDate(), url);
            emailService.sendMail(Arrays.asList("xx@qq.com"), content, SUBJECT);
        } catch (Exception e) {
            log.error("EmailModelServiceImpl#tuneSubmitEmail 执行过程抛出异常 e:{}" + e.getMessage());
    ...
    public void tuneProcessEmail(Integer pipelineId, Integer appId) {
        try {
            String url = buildUrl(pipelineId, appId);
            //todo 获取用户邮箱 发送邮箱通知
            String content = String.format("hello,  %s starts batch tuning at %s,  <a href=\"%s\">please click for details</a>",
    ...
    public void tuneFinishEmail(Integer pipelineId, Integer appId) {
        try {
            String url = buildUrl(pipelineId, appId);
            //todo 获取用户邮箱 发送邮箱通知
            String content = String.format("hello,  %s has finished tuning at %s,  <a href=\"%s\">please click for details</a>",
                    getAppName(appId), getCurrentDate(), url);
            emailService.sendMail(Arrays.asList("xx@qq.com"), content, SUBJECT);
        } catch (Exception e) {
            log.error("EmailModelServiceImpl#tuneSubmitEmail 执行过程抛出异常 e:{}" + e.getMessage());
    ...
    public void tuneCancelEmail(Integer pipelineId, Integer appId) {
        try {
            String url = buildUrl(pipelineId, appId);
            //todo 获取用户邮箱 发送邮箱通知
            String content = String.format("hello,  %s has canceled tuning at %s,  <a href=\"%s\">please click for details</a>",
                    getAppName(appId), getCurrentDate(), url);
            emailService.sendMail(Arrays.asList("xx@qq.com"), content, SUBJECT);
        } catch (Exception e) {
            log.error("EmailModelServiceImpl#tuneSubmitEmail 执行过程抛出异常 e:{}" + e.getMessage());
    ...
    @Override
    public void sendNotifyMsg(List<String> emailList, Integer appId, Integer pipelineId, String subject, String notifyMessage) {
        try {
            String url = buildUrl(pipelineId, appId);
            String content = String.format("hello, %s,  <a href=\"%s\">please click for details</a>", notifyMessage, url);
    ...
    private String getAppName(Integer appId) {
        ...
        if (record == null) {
            throw new RuntimeException(String.format("getAppName occurs an error, appID=[%s]", appId));
        }
    ...
    private String getPlanName(Integer pipelineId) {
        ...
        if(null == tunePipeline){
            throw new RuntimeException(String.format("getPlanName get tunePipeline occurs an error, pipelineId=[%s]", pipelineId));
        }

        TunePlan tunePlan = tunePlanRepository.findTunePlanById(tunePipeline.getTunePlanId());
        if(null == tunePlan){
            throw new RuntimeException(String.format("getPlanName get tunePlan occurs an error, pipelineId=[%s]", pipelineId));
        }
