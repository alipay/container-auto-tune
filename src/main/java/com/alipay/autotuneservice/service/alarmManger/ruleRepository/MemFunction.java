...
/**
 * @author huoyuqi
 * @version MemFunction.java, v 0.1 2022年12月27日 8:02 下午 huoyuqi
 */
@Slf4j
public class MemFunction extends BasicMetricCalcFunction {
    ...
    public AviatorObject call(Map<String, Object> env) {
        try {
            ...
            double sum = metricData.stream().mapToDouble(JvmMonitorMetricData::getJvm_mem_util).sum();
            double lastData = metricData.get(metricData.size() - 1).getJvm_mem_util();
            ...
            Optional.ofNullable(result).ifPresent(e -> {
                e.setResultMessage(e.getStatus() ? String.format("memUtil 在最近%sS超过设置的阈值", ruleModel.getTime() / 1000) : "");
            });
