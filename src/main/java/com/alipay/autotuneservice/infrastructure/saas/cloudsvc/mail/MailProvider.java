 ...
 */
package com.alipay.autotuneservice.infrastructure.saas.cloudsvc.mail;
...
public interface MailProvider {
    ...
    default String getFromMail(String cloudType) {
        try {
            String include = SpringContextUtil.getSdkEnv();
            Map ml = SpringPropertiesCache.get(include);
            // 这里，需要截取掉 -test/-prod, 获取aws/aliyun 下的配置参数
            include = include.substring(0, include.indexOf("-"));
            Map<String, String> mailMap = (HashMap) ((HashMap) ml.get(include)).get("mail");
            return mailMap.get("sendMail");
        } catch (Exception e) {
